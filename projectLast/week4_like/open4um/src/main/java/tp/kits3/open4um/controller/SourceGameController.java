...
public class SourceGameController {
	...
	public String indexPage(Model model) {
		...
		List<SourceDto> listSource2 = sourceservice.selectLimit(num);
		
		//List<category> listCate = categoryservice.getBycategory(parentid);
		model.addAttribute("listSource", listSource);
		int count = (int)Math.ceil(1.0*listSource.size()/3);
	...
	public String indexPage2(Model model, @PathVariable(name="pageNo") int pageNo) {
		...
		int num = (pageNo - 1) * 3;
		List<SourceDto> list = sourceservice.selectLimit(num);
		...
		int count = (int)Math.ceil(1.0 * count2/3);
		model.addAttribute("count", count);
